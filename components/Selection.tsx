...
export default function Selection() {
  return (
    <section
      className="px-6 py-20 bg-alternative"
    >
          ...
          <Reveal key={step.number}>
            <div className="flex gap-6">
              <div className="flex flex-col items-center">
                <div
                  className="w-10 h-10 rounded-full flex items-center justify-center font-bold text-white flex-shrink-0 bg-secondary"
                >
                  {step.number}
                </div>
                {index < steps.length - 1 && (
                  <div
                    className="w-px flex-1 mt-2 bg-secondary/30"
                  />
                )}
              </div>
              <div className="pb-8">
                <h3
                  className="text-lg font-bold mb-2 text-secondary"
                >
                  {step.title}
                </h3>
